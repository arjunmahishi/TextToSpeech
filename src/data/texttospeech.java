	...
	protected void createContents() {
		...
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (in.getSelection()) {
					tts.say(T1.getText(), "hi");
				} else if (br.getSelection()) {
					tts.say(T1.getText(), "en-uk");
				} else if (am.getSelection()) {
					tts.say(T1.getText(), "en-us");
				}
			}
		});

		/*
		 * T1.addKeyListener(new KeyAdapter() { public void keyPressed(KeyEvent
		 * e) { if(e.getKeyCode()==KeyEvent.VK_ENTER) { widgetSelected(); } }
		 * });
		 */
